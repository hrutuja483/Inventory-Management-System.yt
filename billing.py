...
class BillClass:
    def __init__(self,root):
        ...
        Cal_Frame=Frame(Cal_Frame,bd=9,relief=RIDGE,bg="white")
        ...
        cart_Frame=Frame(Cal_Frame,bd=3,relief=RIDGE)
        ...
        footer=Label(self.root,text="Inventory Management System",font=("times new roman",11),bg="#4d636d",fg="white").pack(side=BOTTOM,fill=X)
        self.show()
        #self.bill_top()
    ...
    def search(self):
        ...
        try:
            if self.var_search.get()=="":
                ...
                cur.execute("select pid,name,price,qty,status from product where name LIKE '%"+self.var_search.get()+"%' and status='ACTIVEs'")
                rows=cur.fetchall()
                if len(rows)!=0:
                    self.product_Table.delete(*self.product_Table.get_children())
                    for row in rows:
